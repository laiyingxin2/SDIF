...
dfdc_config= config_dfdc["train_cfg"]
val_dfdc_dataset = DFDC(val_dfdc_config)
dfdc_dataset = DFDC(dfdc_config)
...
dff_config_path = "config/dataset/dff.yml"
with open(dff_config_path) as config_file:
    config_dff = yaml.load(config_file, Loader=yaml.FullLoader)
dff_config = config_dff["train_cfg"]
val_dff_config= config_dff["test_cfg"]

DFF_dataset = DFF(dff_config)
val_DFF_dataset = DFF(val_dff_config)
...
#c23 celeb
combined_dataset2 = ConcatDataset([celeb_dataset, combined_dataset3])
combined_dataset=ConcatDataset([dfdc_dataset, wild_dataset])
...
def parse_args():
    ...
    parser.add_argument('--model_name', default="Xcepbaseline_DFDC_WDF/", type=str)
...
# def validation(model, dataloader, num_imgs):
#     print('Validating...')
#     model.eval()
#     batch_val_losses = []
#     SCORE = np.zeros(num_imgs)
...
# def validation2(model, dataloader, num_imgs, thre):
#     #print('Validating...')
#     model.eval()
#     batch_val_losses = []
#     SCORE = np.zeros(num_imgs)
...
#     val_loss, AUC, ACC = validation(model, val_dataloader, 179000)
    ...
    for epoch in range(args.warm_start_epoch, args.epochs):
        ...
        for step,data in enumerate(tqdm(training_dataloader)):
            ...
            if Global_step % args.save_model == 0:
                ...
                f1.write(val_msg)
                f1.write('\n')
                torch.save(model.state_dict(), res_folder_name + '/ckpt/' + str(epoch)+'.pth')
                # save model
