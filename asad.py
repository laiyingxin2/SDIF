...
combined_dataset=ConcatDataset([c23_dataset, wild_dataset])
...
def parse_args():
    ...
    parser.add_argument('--model_name', default="Xcepbaselineaaa/", type=str)
...
def test(args, model):
    ...
    val_loss, AUC, ACC = val(model, val_dataloader, 179000)
...
def train(args, model):
    ...
    val_dataloader = DataLoader(val_celeb_dataset, batch_size=args.batch_size_val, shuffle=True,drop_last=True)
...
def main(args):
    ...
    #train(args, model)
    model.load_state_dict(torch.load("Training_results/Xception_baseline_celeb_c40/best.pth"))
    test(args,model)
