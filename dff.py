...
class DFF(AbstractDataset):
    ...
    def __init__(self, cfg, seed=2022, transforms=None, transform=None, target_transform=None):
        ...
        super(DFF, self).__init__(cfg, seed, transforms, transform, target_transform)
        print(f"Loading data from 'DFF' of split '{cfg['split']}'"
...
#         pdb.set_trace()
    ...
    def __get_images(self):
        if self.split == 'train':
            ...
            num = None
        a = torch.load(join(self.root, self.split, "real_diffison.pickle"))
        real_images = [os.path.join(self.root, item[0]) for item in a]

        if num is not None:
            real_images = np.random.choice(real_images, num // 3, replace=False)
        real_tgts = [0] * len(real_images)  
        #real_tgts = [torch.tensor(0)] * len(real_images)
        print(f"real: {len(real_tgts)}")
        b = torch.load(join(self.root, self.split, "fake_diffison.pickle"))
        fake_images = [os.path.join(self.root, item[0]) for item in b]
...
if __name__ == '__main__':
    ...
    config_path = "config/dataset/dff.yml"
